...
const StripeCheckout = ({ history }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (payload.error) {
            ...
            // nhận kết quả sau khi thanh toán thành công
            // Gửi data lên tạo order
            createOrder(payload, user.token).then(res => {
                if (res.data.ok) {
                    // Tạo order thành công => empty cart trong local, redux, data
                    localStorage.removeItem("cart")
                    dispatch({
                        type: 'ADD_TO_CART',
                        payload: []
                    })
                    // Reset Coupon trên redux lại false
                    dispatch({
                        type: 'COUPON_APPLIED',
                        payload: false
                    })
